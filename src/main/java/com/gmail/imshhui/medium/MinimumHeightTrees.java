 ...
 *
 * @see <a href="https://leetcode.com/problems/minimum-height-trees/">310. Minimum Height Trees</a>
 * User: liyulin
 * Date: 2019/10/17
 */
public class MinimumHeightTrees {
    public List<Integer> findMinHeightTrees(int n, int[][] edges) {
        ...
        List<Set<Integer>> lists = new ArrayList<>(n);
        for (int i = 0; i < n; i++) {
        ...
        while (n > 2) {
            ...
            for (Integer leaf : leaves) {
                Integer j = lists.get(leaf).iterator().next();
                lists.get(j).remove(leaf);
